...
VkShaderModule vkBase::createShaderModule(const std::vector<char> &code)
{
...
void vkBase::pickPhysicalDevice()
{
...
uint32_t vkBase::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void vkBase::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory)
{
...
VkCommandBuffer vkBase::beginSingleTimeCommands()
{
    VkCommandBufferAllocateInfo allocInfo{
        .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
        .commandPool = m_commandPool,
...
void vkBase::endSingleTimeCommands(VkCommandBuffer commandBuffer)
{
...
void vkBase::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void vkBase::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
VkImageView vkBase::createImageView(VkImage image, VkFormat format)
{
...
void vkBase::loadImageFromFile(VkImage &image, VkDeviceMemory &memory, std::string_view imagePath)
{
    ...
    createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, image,
                memory);
...
VkFormat vkBase::findDepthFormat()
{
...
void vkBase::createDepthResources()
{
    ...
    createImage(m_swapChainExtent.width, m_swapChainExtent.height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_depthImage,
                m_depthImageMemory);
    m_depthImageView = createImageView(m_depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
    transitionImageLayout(m_depthImage, depthFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
}
void vkBase::mouseHandleDerive(int xposIn, int yposIn)
{
}
}
