...
// TODO @material: Create actual Material class implementation
class Material;
...
auto MaterialRegistry::createMaterial(const MaterialTemplate* pTemplate) -> Expected<Material*>
{
    if (!pTemplate)
    {
        return { Result::RuntimeError, "Failed to create material: null template provided" };
    }

    APH_LOG_INFO("Creating material from template '%s'", std::string(pTemplate->getName()).c_str());

    // Create material using ThreadSafeObjectPool
    Material* pMaterial = m_materialPool.allocate(const_cast<MaterialTemplate*>(pTemplate));
    if (!pMaterial)
    {
