...
constexpr std::string_view toString(PresetSamplerType type)
{
    switch (type)
    {
        case PresetSamplerType::eLinearClampMipmap: return "LinearClampMipmap";
        case PresetSamplerType::eLinearWrapMipmap: return "LinearWrapMipmap";
        case PresetSamplerType::eLinearMirrorMipmap: return "LinearMirrorMipmap";
        case PresetSamplerType::eNearestClampMipmap: return "NearestClampMipmap";
        case PresetSamplerType::eNearestWrapMipmap: return "NearestWrapMipmap";
        case PresetSamplerType::eAnisotropicClamp: return "AnisotropicClamp";
        case PresetSamplerType::eAnisotropicWrap: return "AnisotropicWrap";
        case PresetSamplerType::eShadowPcf: return "ShadowPCF";
        case PresetSamplerType::eShadowEsm: return "ShadowESM";
        case PresetSamplerType::eCubemap: return "Cubemap";
        case PresetSamplerType::eCubemapLow: return "CubemapLow";
        case PresetSamplerType::ePointClamp: return "PointClamp";
...
Result SamplerPool::initialize()
{
    ...
    for (size_t i = 0; i < kSamplerTypeCount; ++i)
    {
        ...
        if (!result.success())
        {
            CM_LOG_ERR("Failed to create sampler type %s: %s", 
                toString(static_cast<PresetSamplerType>(i)).data(),
                result.toString());
            return result;
        }
    }
...
Result SamplerPool::createPredefinedSampler(PresetSamplerType type)
{
    ...
    if (!debugName.empty())
    {
        ...
        if (!result.success())
        {
            CM_LOG_WARN("Failed to set debug name for sampler %s: %s", 
                        toString(type).data(), result.toString());
