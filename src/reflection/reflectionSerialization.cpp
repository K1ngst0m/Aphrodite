...
toml::array serializeShaderStages(ShaderStageFlags stages)
{
    ...
    if (stages & ShaderStage::VS) result.push_back("vs");
    if (stages & ShaderStage::TCS) result.push_back("tcs");
    if (stages & ShaderStage::TES) result.push_back("tes");
    if (stages & ShaderStage::GS) result.push_back("gs");
    if (stages & ShaderStage::FS) result.push_back("fs");
    if (stages & ShaderStage::CS) result.push_back("cs");
    if (stages & ShaderStage::TS) result.push_back("ts");
    if (stages & ShaderStage::MS) result.push_back("ms");
...
ShaderStageFlags deserializeShaderStages(const toml::array* arr)
{
    ...
    for (const auto& val : *arr)
    {
        if (val.is_string())
        {
            ...
            if (stage == "vs") result |= ShaderStage::VS;
            else if (stage == "tcs") result |= ShaderStage::TCS;
            else if (stage == "tes") result |= ShaderStage::TES;
            else if (stage == "gs") result |= ShaderStage::GS;
            else if (stage == "fs") result |= ShaderStage::FS;
            else if (stage == "cs") result |= ShaderStage::CS;
            else if (stage == "ts") result |= ShaderStage::TS;
            else if (stage == "ms") result |= ShaderStage::MS;
        }
    }
...
CombinedResourceLayout deserializeCombinedResourceLayout(const toml::table* table)
{
    ...
    if (auto specConstantMasks = table->get_as<toml::table>("specConstantMasks"))
    {
        for (const auto& [stageName, mask] : *specConstantMasks)
        {
            ...
            else if (stageName == "TCS") stage = ShaderStage::TCS;
            else if (stageName == "TES") stage = ShaderStage::TES;
            else if (stageName == "GS") stage = ShaderStage::GS;
            else if (stageName == "FS") stage = ShaderStage::FS;
            else if (stageName == "CS") stage = ShaderStage::CS;
            else if (stageName == "TS") stage = ShaderStage::TS;
...
VertexInput deserializeVertexInput(const toml::array* array)
{
    ...
    if (!result.attributes.empty())
    {
        ...
        for (const auto& attr : result.attributes)
        {
            ...
            switch (attr.format)
            {
                case Format::R32_FLOAT: attrSize = 4; break;
                case Format::RG32_FLOAT: attrSize = 8; break;
                case Format::RGB32_FLOAT: attrSize = 12; break;
                case Format::RGBA32_FLOAT: attrSize = 16; break;
