...
void ResourceBindings::bindBuffer(VulkanBuffer *pBuffer, VkDeviceSize offset, VkDeviceSize range,
                                  uint32_t set, uint32_t binding, uint32_t arrayElement)
{
    bind(set, binding, arrayElement,
         BindingInfo{ offset, range, pBuffer, VK_NULL_HANDLE, VK_NULL_HANDLE, VK_NULL_HANDLE, true });
}

void ResourceBindings::bindBufferView(VulkanBufferView *pBufferView, uint32_t set, uint32_t binding,
                                      uint32_t arrayElement)
{
    bind(set, binding, arrayElement,
         BindingInfo{ 0, 0, VK_NULL_HANDLE, pBufferView, VK_NULL_HANDLE, VK_NULL_HANDLE, true });
}

void ResourceBindings::bindImageView(VulkanImageView *pImageView, VkSampler sampler, uint32_t set,
                                     uint32_t binding, uint32_t arrayElement)
{
    bind(set, binding, arrayElement,
         BindingInfo{ 0, 0, VK_NULL_HANDLE, VK_NULL_HANDLE, pImageView, sampler, true });
}
...
{
    bind(set, binding, arrayElement,
         BindingInfo{ 0, 0, VK_NULL_HANDLE, VK_NULL_HANDLE, VK_NULL_HANDLE, sampler, true });
}
                            ...
                            const BindingInfo &info)
{
    ...
    if(info.pBuffer == VK_NULL_HANDLE && info.pBufferView == VK_NULL_HANDLE &&
       info.pImageView == VK_NULL_HANDLE && info.sampler == VK_NULL_HANDLE)
    {
