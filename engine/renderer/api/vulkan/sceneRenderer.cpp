...
namespace
{
VulkanDescriptorSetLayout *samplerLayout = nullptr;
...
VulkanPipeline *CreateShadowPipeline(VulkanDevice *pDevice, VulkanRenderPass *pRenderPass)
{
    ...
    // scene
    {
        std::vector<VkDescriptorSetLayoutBinding> bindings{
            vkl::init::descriptorSetLayoutBinding(
...

    {
        ...
        effectInfo.shaderMapList[VK_SHADER_STAGE_VERTEX_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "shadow.vert.spv");
        effectInfo.shaderMapList[VK_SHADER_STAGE_FRAGMENT_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "shadow.frag.spv");

        GraphicsPipelineCreateInfo pipelineCreateInfo{};
        pipelineCreateInfo.vertexInputInfo =
            pipelineCreateInfo.vertexInputBuilder.getPipelineVertexInputState(
                { VertexComponent::POSITION });
        VK_CHECK_RESULT(
            pDevice->createGraphicsPipeline(&pipelineCreateInfo, &effectInfo, pRenderPass, &pipeline));
    }
...
VulkanPipeline *CreatePostFxPipeline(VulkanDevice *pDevice, VulkanRenderPass *pRenderPass)
{
    ...
    {
        std::vector<VkDescriptorSetLayoutBinding> bindings;
        bindings.push_back(vkl::init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
...

    {
        ...
        effectInfo.setLayouts.push_back(samplerLayout);
        effectInfo.shaderMapList[VK_SHADER_STAGE_VERTEX_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "postFX.vert.spv");
        effectInfo.shaderMapList[VK_SHADER_STAGE_FRAGMENT_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "postFX.frag.spv");
...
VulkanPipeline *CreateSkyboxPipeline(VulkanDevice *pDevice, VulkanRenderPass *pRenderPass)
{
    ...
    {
        std::vector<VkDescriptorSetLayoutBinding> bindings;
        bindings.push_back(vkl::init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
    ...
    {
        std::vector<VkDescriptorSetLayoutBinding> bindings;
        bindings.push_back(vkl::init::descriptorSetLayoutBinding(
...

    {
        ...
        effectInfo.constants.push_back(
            vkl::init::pushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, sizeof(glm::mat4), 0));
        effectInfo.shaderMapList[VK_SHADER_STAGE_VERTEX_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "skybox.vert.spv");
        effectInfo.shaderMapList[VK_SHADER_STAGE_FRAGMENT_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "skybox.frag.spv");
...
VulkanPipeline *CreateForwardPipeline(VulkanDevice *pDevice, VulkanRenderPass *pRenderPass,
                                      SceneInfo &sceneInfo)
{
    ...
    // scene
    {
        std::vector<VkDescriptorSetLayoutBinding> bindings{
            ...
            vkl::init::descriptorSetLayoutBinding(
                VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 1, sceneInfo.cameraCount),
            vkl::init::descriptorSetLayoutBinding(
                VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 2, sceneInfo.lightCount),
        };
        VkDescriptorSetLayoutCreateInfo createInfo = vkl::init::descriptorSetLayoutCreateInfo(bindings);
        pDevice->createDescriptorSetLayout(&createInfo, &sceneLayout);
    }
    ...
    // material
    {
        std::vector<VkDescriptorSetLayoutBinding> bindings{
            vkl::init::descriptorSetLayoutBinding(
                VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
                VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0),  // material info
            vkl::init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
    ...
    {
        std::vector<VkDescriptorSetLayoutBinding> bindings{
            vkl::init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_SAMPLER,
                                                  VK_SHADER_STAGE_FRAGMENT_BIT, 0),
            vkl::init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_SAMPLER,
                                                  VK_SHADER_STAGE_FRAGMENT_BIT, 1),
        };
        ...
        VkDescriptorSetLayoutCreateInfo createInfo = vkl::init::descriptorSetLayoutCreateInfo(bindings);
        pDevice->createDescriptorSetLayout(&createInfo, &samplerLayout);
    }

    {
        ...
        effectInfo.constants.push_back(
            vkl::init::pushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, sizeof(glm::mat4), 0));
        effectInfo.shaderMapList[VK_SHADER_STAGE_VERTEX_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "pbr.vert.spv");
        effectInfo.shaderMapList[VK_SHADER_STAGE_FRAGMENT_BIT] =
            pDevice->getShaderCache()->getShaders(shaderDir / "pbr.frag.spv");
...
std::unique_ptr<VulkanSceneRenderer> VulkanSceneRenderer::Create(
...
void VulkanSceneRenderer::cleanupResources()
{
    ...
    vkDestroySampler(m_pDevice->getHandle(), m_samplers.postFX, nullptr);
    vkDestroySampler(m_pDevice->getHandle(), m_samplers.texture, nullptr);
...
void VulkanSceneRenderer::drawScene()
{
    ...
    commandBuffer->cmdBindDescriptorSet(m_forwardPass.pipeline, PASS_FORWARD::SET_SAMPLER, 1, &m_samplers.set);
    ...
    commandBuffer->cmdBindDescriptorSet(m_postFxPass.pipeline, PASS_POSTFX::SET_SAMPLER, 1, &m_samplers.set);
...
void VulkanSceneRenderer::_initRenderData()
{
    ...
    for(auto &set : m_sceneSets)
    {
        std::vector<VkWriteDescriptorSet> writes{
            vkl::init::writeDescriptorSet(set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
                                          &m_sceneInfoUB->getBufferInfo()),
            vkl::init::writeDescriptorSet(set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
                                          m_cameraInfos.data(), m_cameraInfos.size()),
    ...
    for(auto &renderable : m_renderList)
    {
        ...
        for(auto &material : m_scene->getMaterials())
        {
            ...
            auto set = m_forwardPass.pipeline->getDescriptorSetLayout(PASS_FORWARD::SET_MATERIAL)
                           ->allocateSet();
            VulkanBuffer *matInfoUB = nullptr;
            {
                ...
                descriptorWrites.push_back(vkl::init::writeDescriptorSet(
                    set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &matInfoUB->getBufferInfo()));
                ...
                if(bindingBits & MATERIAL_BINDING_BASECOLOR)
                {
                    ...
                    {
                        descriptorWrites.push_back(vkl::init::writeDescriptorSet(
                            set, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1,
                ...
                if(bindingBits & MATERIAL_BINDING_NORMAL)
                {
                    ...
                    {
                        descriptorWrites.push_back(vkl::init::writeDescriptorSet(
                            set, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 2,
                ...
                if(bindingBits & MATERIAL_BINDING_AO)
                {
                    ...
                    {
                        descriptorWrites.push_back(vkl::init::writeDescriptorSet(
                            set, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 4,
                ...
                if(bindingBits & MATERIAL_BINDING_EMISSIVE)
                {
                    ...
                    {
                        descriptorWrites.push_back(vkl::init::writeDescriptorSet(
                            set, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 5,
...
void VulkanSceneRenderer::_initSkyboxResource()
{
    ...
    std::vector<VkWriteDescriptorSet> writeDescriptorSets{
        ...
        vkl::init::writeDescriptorSet(set, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, m_cameraInfos.data()),
...
void VulkanSceneRenderer::_initPostFx()
{
    ...
    for(auto idx = 0; idx < imageCount; idx++)
    {
...

        {
            ...
            VK_CHECK_RESULT(m_pDevice->createFramebuffers(&createInfo, &framebuffer, attachments.size(),
                                                          attachments.data()));
        }
    }

    {
        ...
        VK_CHECK_RESULT(
            m_pDevice->createRenderPass(nullptr, &m_postFxPass.renderPass, colorAttachments));
    }
    ...
    for(uint32_t idx = 0; idx < imageCount; idx++)
    {
        ...
        vkUpdateDescriptorSets(m_pDevice->getHandle(), static_cast<uint32_t>(writes.size()),
                               writes.data(), 0, nullptr);
    }

    // TODO
    {
        auto &set = m_samplers.set;
        set = m_postFxPass.pipeline->getDescriptorSetLayout(PASS_POSTFX::SET_SAMPLER)->allocateSet();
        ...
        std::vector<VkWriteDescriptorSet> writes{
            vkl::init::writeDescriptorSet(set, VK_DESCRIPTOR_TYPE_SAMPLER, 0, imageInfos.data(),
                                          imageInfos.size()),
        };
        vkUpdateDescriptorSets(m_pDevice->getHandle(), static_cast<uint32_t>(writes.size()),
                               writes.data(), 0, nullptr);
...
void VulkanSceneRenderer::_initForward()
{
...

    {
        ...
        VK_CHECK_RESULT(m_pDevice->createRenderPass(nullptr, &m_forwardPass.renderPass, colorAttachments,
                                                    depthAttachment));
    }
...
void VulkanSceneRenderer::_initSampler()
{
    // texture
    {
        ...
        VK_CHECK_RESULT(
            vkCreateSampler(m_pDevice->getHandle(), &samplerInfo, nullptr, &m_samplers.texture));
    }
    ...
    // shadow
    {
        ...
        VK_CHECK_RESULT(
            vkCreateSampler(m_pDevice->getHandle(), &createInfo, nullptr, &m_samplers.shadow));
    }
    ...
    // postFX
    {
        VkSamplerCreateInfo createInfo = vkl::init::samplerCreateInfo();
        VK_CHECK_RESULT(
            vkCreateSampler(m_pDevice->getHandle(), &createInfo, nullptr, &m_samplers.postFX));
    }
}
}  // namespace vkl
