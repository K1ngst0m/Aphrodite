...
namespace aph::vk
{
SceneRenderer::SceneRenderer(WSI* wsi, const RenderConfig& config)
    : Renderer(wsi, config)
{
}
...
void SceneRenderer::_initSet()
{
    ...
    // geometry
    {
        ...
        std::vector<VkWriteDescriptorSet> writes{
            init::writeDescriptorSet(m_geometrySet->getHandle(), VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
                                     &transformBufferInfo),
            init::writeDescriptorSet(m_geometrySet->getHandle(), VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 2,
                                     &cameraBufferInfo),
            init::writeDescriptorSet(m_geometrySet->getHandle(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 4,
                                     textureInfos.data(), textureInfos.size()),
            init::writeDescriptorSet(m_geometrySet->getHandle(), VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 5,
                                     &materialBufferInfo),
        };
        vkUpdateDescriptorSets(m_pDevice->getHandle(), writes.size(), writes.data(), 0, nullptr);
    }
    ...
    // postfx
    {
        ...
        for (std::size_t idx = 0; idx < m_config.maxFrames; idx++)
        {
            VkDescriptorImageInfo inputImageInfo{ .imageView =
                                                      m_images[IMAGE_GENERAL_COLOR][idx]->getView()->getHandle(),
            ...
            std::vector<VkWriteDescriptorSet> writes{
                init::writeDescriptorSet(m_postFxSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0,
                                         &inputImageInfo),
                init::writeDescriptorSet(m_postFxSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
                                         &outputImageInfo),
            };
            vkUpdateDescriptorSets(m_pDevice->getHandle(), writes.size(), writes.data(), 0, nullptr);
        }
    ...
    // gbuffer
    {
        ...
        for (std::size_t idx = 0; idx < m_config.maxFrames; idx++)
        {
            ...
            std::vector<VkWriteDescriptorSet> writes{
                init::writeDescriptorSet(m_gbufferSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 0,
                                         &posImageInfo),
                init::writeDescriptorSet(m_gbufferSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1,
                                         &normalImageInfo),
                init::writeDescriptorSet(m_gbufferSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 2,
                                         &albedoImageInfo),
                init::writeDescriptorSet(m_gbufferSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 3,
                                         &mraoImageInfo),
                init::writeDescriptorSet(m_gbufferSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 4,
                                         &emissiveImageInfo),
                init::writeDescriptorSet(m_gbufferSets[idx]->getHandle(), VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 5,
                                         &shadowMapInfo),
            };
    ...
    // shadow
    {
        ...
        std::vector<VkWriteDescriptorSet> writes{
            init::writeDescriptorSet(m_shadowSet->getHandle(), VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
                                     &transformBufferInfo),
...
{
    m_scene->getRootNode()->traversalChildren(
        [&](SceneNode* node)
        {
...
void SceneRenderer::_initGbuffer()
{
    ...
    for (auto idx = 0; idx < m_config.maxFrames; idx++)
    {
...

        {
            ...
            m_pDevice->executeSingleCommands(QueueType::Graphics,
                                             [&](auto* cmd)
                                             {
...
void SceneRenderer::_initGpuResources()
{
    ...
    // create index buffer
    {
        ...
        aph::BufferLoadInfo loadInfo{ .data = indicesList.data(),
                                      .createInfo = {
                                          .size = static_cast<uint32_t>(indicesList.size() * sizeof(indicesList[0])),
    ...
    // create vertex buffer
    {
        ...
        aph::BufferLoadInfo loadInfo{ .data = verticesList.data(),
                                      .createInfo = {
                                          .size = static_cast<uint32_t>(verticesList.size() * sizeof(verticesList[0])),
    ...
    // create material buffer
    {
        ...
        aph::BufferLoadInfo loadInfo{ .data = materials.data(),
                                      .createInfo = {
                                          .size = static_cast<uint32_t>(materials.size() * sizeof(materials[0])),
                                          .usage = VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT } };
...
void SceneRenderer::_initSkybox()
{
    ...
    // skybox vertex
    {
        ...
        // create vertex buffer
        {
            aph::BufferLoadInfo loadInfo{ .data = (void*)skyboxVertices.data(),
                                          .createInfo = { .size = static_cast<uint32_t>(skyboxVertices.size() *
                                                                                        sizeof(skyboxVertices[0])),
                                                          .usage = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT } };
...
void SceneRenderer::recordPostFX(CommandBuffer* pCommandBuffer)
{
    // post fx
    {
        ...
        {
            VkDescriptorImageInfo inputImageInfo{ .imageView =
                                                      m_images[IMAGE_GENERAL_COLOR][m_frameIdx]->getView()->getHandle(),
                                                  .imageLayout = VK_IMAGE_LAYOUT_GENERAL };
            VkDescriptorImageInfo outputImageInfo{ .imageView = m_pSwapChain->getImage()->getView()->getHandle(),
                                                   .imageLayout = VK_IMAGE_LAYOUT_GENERAL };
            std::vector<VkWriteDescriptorSet> writes = {
...
void SceneRenderer::_initShadow()
{
    ...
    for (auto idx = 0; idx < m_config.maxFrames; idx++)
    {
        ...
        m_pDevice->executeSingleCommands(
            QueueType::Graphics,
            [&](auto* cmd) { cmd->transitionImageLayout(depth, VK_IMAGE_LAYOUT_DEPTH_ATTACHMENT_OPTIMAL); });
    }
