...
std::shared_ptr<Entity> Scene::createEntityFromFile(const std::string &path)
{
    ...
    if(fileLoaded)
    {
        // loadImages(entity->m_images, inputModel);
        // loadMaterials(entity->m_materials, inputModel, 0);
        uint32_t imageOffset = m_images.size();
        uint32_t materialOffset = m_materials.size();
        std::vector<std::shared_ptr<ImageDesc>> images;
        std::vector<std::shared_ptr<Material>> materials;
        ...
        {
            const tinygltf::Node& inputNode = inputModel.nodes[nodeIdx];
            loadNodes(inputNode, inputModel, entity->m_rootNode->createChildNode(), 0);
        }
    }
    else
    {
