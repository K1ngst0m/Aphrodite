...
// inadvertantly damage some of the memory used by this memory tracking software, causing it to crash in much the same
// way that a damaged heap would affect the standard allocation routines.
//
...
static const char*  allocationTypes[] = {"Unknown", "new",    "new[]",    "malloc", "calloc",
    ...
    for(auto ptr : hashTable)
    {
        while(ptr)
        {
            fprintf(fp, "%06d 0x%08X 0x%08X 0x%08X 0x%08X 0x%08X %-8s    %c       %c    %s\r\n", ptr->allocationNumber,
                    ...
                    reinterpret_cast<size_t>(ptr->actualAddress), ptr->actualSize, m_calcUnused(ptr),
...
void* m_allocator(const char* sourceFile, const unsigned int sourceLine, const char* sourceFunc,
                  const unsigned int allocationType, size_t alignment, const size_t reportedSize)
{
    if(cleanupLogOnFirstRun)
    {
    ...
    if(alwaysLogAll)
        log("[+] ---->             addr 0x%08X", reinterpret_cast<size_t>(au->reportedAddress));
...
void* m_reallocator(const char* sourceFile, const unsigned int sourceLine, const char* sourceFunc,
                    const unsigned int reallocationType, size_t reportedSize, void* reportedAddress)
{
    ...
    if(!reportedAddress)
    {
        return m_allocator(sourceFile, sourceLine, sourceFunc, reallocationType, sizeof(void*), reportedSize);
    }
    ...
    // Case where the new address has different alignment in which case we need to copy the old data as to respect
    // realloc guarantees
    if(offset != au->offset)
    {
        // Copy old data
                   ...
                   const unsigned int deallocationType, const void* reportedAddress)
{
    ...
    // delete will both bail before they get here.) So, since ANSI allows free(NULL), we'll not bother trying to
    // actually free the allocated memory or track it any further.
