...
// provide support for custom formatting by
// concatenating the formatting string instead of having the format inside the default message
...
#define APH_INTERNAL_ASSERT_WITH_MSG(type, check, ...) \
    APH_INTERNAL_ASSERT_IMPL(type, check, "Assertion failed: {0}", __VA_ARGS__)

#define APH_INTERNAL_ASSERT_NO_MSG(type, check)                                   \
    APH_INTERNAL_ASSERT_IMPL(type, check, "Assertion '{0}' failed at {1}:{2}",    \
                             APH_STRINGIFY_MACRO(check),                          \
                             std::filesystem::path(__FILE__).filename().string(), \
                             __LINE__)

#define APH_INTERNAL_ASSERT_GET_MACRO_NAME(arg1, arg2, macro, ...) macro
#define APH_INTERNAL_ASSERT_GET_MACRO(...) \
    APH_EXPAND_MACRO(APH_INTERNAL_ASSERT_GET_MACRO_NAME(__VA_ARGS__, \
                                               APH_INTERNAL_ASSERT_WITH_MSG, \
                                               APH_INTERNAL_ASSERT_NO_MSG))
