    ...
    if type(url) is list:
        for url_option in url:
            ...
            try:
                ...
                print("URL Error encountered: {e.reason}. Proceeding with backup...\n\n")
                os.remove(filepath)
                pass
            except urllib.error.HTTPError as e:
                print("HTTP Error  encountered: {e.code}. Proceeding with backup...\n\n")
                os.remove(filepath)
                pass
            except:
                ...
                os.remove(filepath)
                pass
        raise ValueError("Failed to download {filepath}")
...
def UnzipFile(filepath, deleteZipFile=True):
    ...
    with ZipFile(zipFilePath, 'r') as zipFileFolder:
        ...
        for zippedFileName, zippedFileSize in zipFileContent.items():
            UnzippedFilePath = os.path.abspath("{zipFileLocation}/{zippedFileName}")
